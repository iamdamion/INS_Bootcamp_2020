...
def main(argv=sys.argv):
    ...
    arg_parser.add_argument('-year', action='store', type=int,
                            required=False,
                            help='Highlight articles published *AFTER* provided [YEAR] with darker color. (DEFAULT: None)',
                            default=None,
                            dest='year'
                            )
    ...
    def date_puller(item):
        return re.split('(\d+)',item)[1]
    ...
    def make_mat(authors_df):
        ...
        # Sort for year highlight if year provided in args
        if args.year:
            for x in [MM,MW,WM,WW,U]:
                x.sort(key=date_puller)
        else:
            pass
